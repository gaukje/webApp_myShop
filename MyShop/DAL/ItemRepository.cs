...
public class ItemRepository : IItemRepository
{
    ...
    public async Task<Item?> GetItemById(int id)
    {
        ...
        {
            _logger.LogError("[ItemRepository] item FindAsync(id) failed when GetItemById for ItemId" +
                " {ItemId:0000}, error message: {e}", id, e.Message);
            return null;
        }
    }
    ...
    public async Task<bool> Update(Item item)
    {
        ...
        {
            _logger.LogError("[ItemRepository] item FindAsync(id) failed when updating the ItemId {ItemId:0000}," +
